...
// Free app1 header
static void freeApp1Header(APP1Header* header) {
	if (header) {
		free(header);
	}
}
...
static RawIFDArray* createInternalIFDArray() {
...
// Read raw ifd array
static EXIF_STATUS fillInternalIfdArray(FILE* image, RawIFDArray* array) {
	if (!image) {
...
// Create normal EXIF table

// Fill normal exif table

// Free normal exif table